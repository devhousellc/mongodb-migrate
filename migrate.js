...
const path                             = require('path'),
      ...
      { updateFixtures, loadFixtures } = require('./fixtures-migration');
...
let config      = require('config'),
    argsManager = new ArgsManager(process.argv, config, process.cwd());
...
} else if (argsManager.findArg('fixtures')) {
  process.env.NODE_ENV='testing';

  askAboutConnection()
    .then(() => loadFixtures(argsManager.findKey('--configConnection')))
    .then(() => up(argsManager.findKey('up')))
    .then(() => updateFixtures())
    .then(() => {
      process.env.NODE_CONFIG_DIR = path.resolve(process.cwd(), CONFIG_DIR);
